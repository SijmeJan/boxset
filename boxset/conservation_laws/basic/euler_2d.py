...
def _primitive_variables(conserved_variables):
    ...
    prim[3] = (gamma - 1.0)*(conserved_variables[3] -
                             0.5*(conserved_variables[1]**2 +
                                  conserved_variables[2]**2)/prim[0])
...
def _multiply_with_right_eigenvectors_x(primitive_variables, state_vector):
    ...
    ret[1] = (prim[1] - c)*state_vector[0] +\
        ...
        (prim[1] + c)*state_vector[3]
    ret[2] = prim[2]*ret[0] + state_vector[2]
    ret[3] = (h - c*prim[1])*state_vector[0] +\
        ekin*state_vector[1] + prim[2]*state_vector[2] +\
        (h + c*prim[1])*state_vector[3]
...
def _multiply_with_right_eigenvectors_y(primitive_variables, state_vector):
    ...
    ret[2] = (prim[2] - c)*state_vector[0] + \
        prim[2]*(0*state_vector[1] + state_vector[2]) + \
        (prim[2] + c)*state_vector[3]
    ret[3] = (h - c*prim[2])*state_vector[0] +\
        ekin*state_vector[2] + prim[1]*state_vector[1] +\
        (h + c*prim[2])*state_vector[3]
...
def _max_wave_speed_x(state_vector):
    prim = _primitive_variables(state_vector)
    return np.abs(prim[1]) + np.sqrt(gamma*prim[3]/prim[0])


def _max_wave_speed_y(state_vector):
    prim = _primitive_variables(state_vector)
    return np.abs(prim[2]) + np.sqrt(gamma*prim[3]/prim[0])
