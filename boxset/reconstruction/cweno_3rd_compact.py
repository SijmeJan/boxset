...
def interpolate_to_edge(U, X):
    ...
    X is the evaluation point: X=2 for x_{i+1/2}.
    ...
    UC = (u + ux*(X - 1.5) + 0.5*uxx*(X - 1.5)**2 -
          ideal_weights[0]*UL - ideal_weights[2]*UR)/ideal_weights[1]
...
def smoothness_coefficients(U):
    '''
    Last dimension of U should have 3 elements.
    Returns the smoothness coefficients for the left and
    right-biased stencils,as well as central
    '''

    betaL = (U[..., 0] - U[..., 1])**2
    betaR = (U[..., 2] - U[..., 1])**2
    betaC = 13*(U[..., 2] - 2*U[..., 1] + U[..., 0])**2/3 + \
        0.25*(U[..., 2] - U[..., 0])**2
