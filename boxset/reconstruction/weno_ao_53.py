...
def interpolate_to_edge(U, X):
    ...
    X is the evaluation point: X=3 for x_{i+1/2}.
    Returns estimates for U using the three-point left-, centre- and
    right-biased stencils, as well as an estimate based on the
...
def smoothness_coefficients(U):
    ...
    right-biased stencils, as well as the smoothness coefficient for the
    5-point centered stencil.
    '''
    betaL = 13*(U[..., 0] - 2*U[..., 1] + U[..., 2])**2/12 + \
        0.25*(U[..., 0] - 4*U[..., 1] + 3*U[..., 2])**2
    betaC = 13*(U[..., 1] - 2*U[..., 2] + U[..., 3])**2/12 + \
        0.25*(U[..., 1] - U[..., 3])**2
    betaR = 13*(U[..., 2] - 2*U[..., 3] + U[..., 4])**2/12 + \
        0.25*(3*U[..., 2] - 4*U[..., 3] + U[..., 4])**2
...
def calc_interface_flux(U, X, epsilon=1.0e-12):
    ...
    return wL*uL + wC*uC + wR*uR + \
        w5*(u5 - gamma1_3*uL - gamma2_3*uC - gamma3_3*uR)/gamma3_5
